...
xtitle = r"#Lambda_{T} ADD${KKconv})"
ytitle = r"95% CL Limit on #sigma/#sigma_{ADD}" #"Signal Strength"
lumi_val = ${intlumi}

x_13,y_th_13,y_13_obs,y_th = array( 'd' ), array( 'd' ), array( 'd' ), array( 'd' )

LambdaT=${LambdaT_list}
nbins = len(LambdaT)
xsecInPb = 1.00
i = 0
while i < nbins:
...
for lumi in [str(lumi_val)]:
	...
	c=ROOT.TCanvas('ADD${KKconv}','',1200,1000)
	c.SetLogy()
	margine=0.15
	...
	for i in range(len(lines_exp)):
		print LambdaT[i],'&',rounding(lines_exp[i][2]),'&',rounding(lines_exp[i][4]),'&',rounding(lines_exp[i][1]),'&',rounding(lines_exp[i][5]),'&',rounding(lines_exp[i][3]),'\\\\'
	...
	# Bin Label
	for i, limit in enumerate(LambdaT):
		exp1sigma.GetXaxis().SetBinLabel(i+1, str(LambdaT[i]))
	...
	legend=ROOT.TLegend(0.335,0.55,0.9,0.9)
 	...
 	if not os.path.exists('../limitplots'):
        	os.mkdir('../limitplots')
	c.SaveAs('../limitplots/ADD${KKconv}.pdf')
