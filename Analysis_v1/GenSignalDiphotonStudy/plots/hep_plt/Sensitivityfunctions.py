...
def CalcSensitivityADD(obj, DATASET, labels, lumi=None, McutList=None, MassRange=None, chiMax=None):
	f = open("ADDsensitivity_log.csv", "w+")
	
	histClones, Mcuts = [], []
        print "Mass Range: ", MassRange, "chidiphoton Range: 0-", chiMax	
	...
	for histclone, label in zip(histClones, labels):
		if "GG" in label or "SM" in label:
			histSM = histclone
			if lumi is not None:
				histSM.Scale(lumi)
			for mcut in Mcuts:
				xmin, xmax = RangeByVar(obj, mcut, chiMax=chiMax)
		...
		if "ADD" in label:
			...
			f.write(yieldsInfo)
def CalcSigADD(obj, DATASET, labels, lumi=None, McutList=None, MassRange=None, chiMax=None):	
	histClones, Mcuts = [], []
        print "Mass Range: ", MassRange, "chidiphoton Range: 0-", chiMax	
	for dset, label in zip(DATASET, labels):
		if "SM" in label or "GG" in label:
	...
	print "Variable: ", obj 
	h = "Model Pt, mcut, B, S+B, S, S95CL, Signal Region, S/sqrt(B), S/sqrt(S+B)"
	#print h.split(',')
	
	infoList = []
	infoList.append(h.split(','))
	for histclone, label in zip(histClones, labels):
		if "GG" in label or "SM" in label:
			histSM = histclone
			if lumi is not None:
		...
		if "ADD" in label:
			...
			for mcut in Mcuts:
				...
				s    = round(s, 1)
				cl95 = round(cl95, 1)
				Zb   = round(s/TMath.Sqrt(b), 1)
				Zsb  = round(s/TMath.Sqrt(s+b), 1) 
			siginfo = lbel, mcut, b, sb, s, cl95, r, Zb, Zsb
			infoList.append(siginfo)

	for inf in infoList:
		row_format ="{:>15}" * (len(inf) + 1)
		print row_format.format("", *inf)
