		...
		#print label
			...
			if "ADD" in label:
				if "Sherpa" in label:
					...
					leglabel = r"MS=%s, KK-%s" %(LambdaU, KK)
			...
			if "Unp" in label:
				PH, spin, du, LU, pT = label
	      			leglabel = r"MS=%s, d_{u}=%s, spin-%s" %(LU, du, spin)
...
def PlotRatio(h1, h2, label1, label2, tag, xRange=None, yratioRange=None, binning=None, h3=None):
	...
	y = hRatio.GetYaxis()
	y.SetRangeUser(0, 10)
	if yratioRange is not None:
		yrmin, yrmax = yratioRange
		y.SetRangeUser(yrmin, yrmax)
	y.SetTitle("Ratio")
	y.SetTitleSize(20)
	y.SetTitleFont(43)
	x = hRatio.GetXaxis()
...
# Unedited
def hStackRatio(obj, histList, labelList, tag=None, lumi=None, Mrange=None):
    # Create Canvas
    c, pad1, pad2 = createCanvasPads()

    # Other Settings
    c, xtitle, ytitle, xmin, xmax, legpos = LabelMaker(obj, c)
    if "Minv" in obj and Mrange is not None:
	xmin, xmax = Mrange
    ...
    # Legend
    leg = makeLegend(legpos, legendtitle="#bf{Sensitivity Studies}")
    h1 = histList[0].Clone("h1")
    h2 = histList[1].Clone("h2")
    if lumi is not None:
	# To preserve type as TH1D
    ...
    # Draw legend
    leg.AddEntry(hist1, labelList[0])
    leg.AddEntry(hist2, labelList[1])
    leg.Draw()

    #--------- Ratio pad
    hRatio = createRatio(hist1, hist2)
    pad2.cd()
    hist1.GetYaxis().SetTitle("ratio %s-%s" %(labelList[0], labelList[1]))
    x = hRatio.GetXaxis()
    x.SetRangeUser(xmin, xmax)
    ...
    if lumi is not None:
    	#set_CMS_lumi(, 4, 11, lumi)
	set_CMS_lumi(c, 4, 0, lumi)
    if lumi is None:
	set_CMS_lumi(c, 4, 0, 1)
	c.Update()
	c.Draw()
    outName = "Ratio"
    if tag is not None:
	outName = tag + outName
    	...
    	os.chdir(outputdir)
    	print "Saving in %s" %(outputdir)
    	c.SaveAs(outName+"V%s.pdf"%(obj))
    	os.chdir("..")

    else:
	c.Print(outName+"V%s.pdf"%(obj))
