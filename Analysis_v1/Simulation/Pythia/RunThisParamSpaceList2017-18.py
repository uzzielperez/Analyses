...
for value in LambdaT_lst:
	...
	while rangeIndex < len(minMasslist):
		...
		process       = sp.Popen(command.split(), stdout=sp.PIPE)
	        ...
	        dataset = 'ADDGravToGG_NegInt-%d_LambdaT-%d_M-%dTo%d_%dTeV-pythia8' %(negInt, int(lambdaT), minMass, maxMass, COM)
	        ...
	        genfragname = 'ADDGravToGG_NegInt-%d_LambdaT-%d_M-%dTo%d_%dTeV-pythia8_MASTER_cff.py' %(negInt, int(lambdaT), minMass, maxMass, COM)
